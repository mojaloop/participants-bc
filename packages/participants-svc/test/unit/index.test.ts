...
describe('Participant Service Test', () => {
  ...
  test("should call handleSettlementMatrixSettledEvt() method of ParticipantAggregate when message.msgName is SettlementMatrixSettledEvt", async () => {
    // Arrange
    const fakeMessage = {
      msgName: "SettlementMatrixSettledEvt",
      msgKey: "mocked_key",
      msgId: "mocked_id",
    };
    const handleSettlementMatrixSettledEvtSpy = jest.spyOn(participantAggMock, "handleSettlementMatrixSettledEvt");

    // Act
    await mockedEventHandler["_msgHandler"](fakeMessage as any);
    
    // Assert
    expect(handleSettlementMatrixSettledEvtSpy).toBeCalledTimes(1);
