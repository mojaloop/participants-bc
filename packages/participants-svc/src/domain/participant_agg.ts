...
enum AuditedActionNames {
    ...
    PARTICIPANT_CERTIFICATE_REVOKED = "PARTICIPANT_CERTIFICATE_REVOKED",
}
...
export class ParticipantAggregate {
        ...
        for (const currency of this._currencyList) {
            ...
            try {
                ...
                hubParticipant.changeLog.push({
                    ...
                    notes: `(participants-svc bootstrap routine added HMLNS account for: ${currency.code})`,
                });
            } catch (err) {
            ...
            try {
                ...
                hubParticipant.changeLog.push({
                    ...
                    user: "(system)",
                    timestamp: hubParticipant.createdDate,
                    notes: `(participants-svc bootstrap routine added Reconciliation account for: ${currency.code})`,
                });
            } catch (err) {
                this._logger.error(err);
            ...
            if (pendingApprovals) {
                    ...
                    for (const changeReq of pendingApprovals.accountsChangeRequest) {
                        try {
                            ...
                            messages.push({
                                reqId: changeReq.id,
                                status: "error",
                                message: `Failed to approve request Id: ${changeReq.id} of ${changeReq.participantId}. ${(err as Error).message}`,
                            });
                        }
                    }
                ...
                if (pendingApprovals.fundsMovementRequest) {
                    for (const changeReq of pendingApprovals.fundsMovementRequest) {
                        try {
                            await this.approveFundsMovement(secCtx, changeReq.participantId, changeReq.id);
                            messages.push({
                                reqId: changeReq.id,
                                status: "success",
                                message: `Successfully approved request id: ${changeReq.id} of ${changeReq.participantId}`
                ...
                if (pendingApprovals.ndcChangeRequests) {
                    for (const changeReq of pendingApprovals.ndcChangeRequests) {
                        try {
                            await this.approveParticipantNetDebitCap(secCtx, changeReq.participantId, changeReq.id);
                            messages.push({
                                reqId: changeReq.id,
                                status: "success",
                                message: `Successfully approved request id: ${changeReq.id} of ${changeReq.participantId}`,
                ...
                if (pendingApprovals.ipChangeRequests) {
                    for (const changeReq of pendingApprovals.ipChangeRequests) {
                        try {
                            await this.approveParticipantSourceIpChangeRequest(secCtx, changeReq.participantId, changeReq.id);
                            messages.push({
                                reqId: changeReq.id,
                                status: "success",
                                message: `Successfully approved request id: ${changeReq.id} of ${changeReq.participantId}`,
                ...
                if (pendingApprovals.contactInfoChangeRequests) {
                    for (const changeReq of pendingApprovals.contactInfoChangeRequests) {
                        try {
                            await this.approveParticipantContactInfoChangeRequest(secCtx, changeReq.participantId, changeReq.id);
                            messages.push({
                                reqId: changeReq.id,
                                status: "success",
                                message: `Successfully approved request id: ${changeReq.id} of ${changeReq.participantId}`,
        ...
        try {
            ...
            if (pendingApprovals) {
                    ...
                    for (const changeReq of pendingApprovals.accountsChangeRequest) {
                        try {
                            ...
                            messages.push({
                                reqId: changeReq.id,
                                status: "error",
                                message: `Failed to reject request Id: ${changeReq.id} of ${changeReq.participantId}. ${(err as Error).message}`,
                            });
                        }
                    }
                ...
                if (pendingApprovals.fundsMovementRequest) {
                    for (const changeReq of pendingApprovals.fundsMovementRequest) {
                        try {
                            await this.rejectFundsMovement(secCtx, changeReq.participantId, changeReq.id);
                            messages.push({
                                reqId: changeReq.id,
                                status: "success",
                                message: `Successfully rejected request id: ${changeReq.id} of ${changeReq.participantId}`
                ...
                if (pendingApprovals.ndcChangeRequests) {
                    for (const changeReq of pendingApprovals.ndcChangeRequests) {
                        try {
                            await this.rejectParticipantNetDebitCap(secCtx, changeReq.participantId, changeReq.id);
                            messages.push({
                                reqId: changeReq.id,
                                status: "success",
                                message: `Successfully rejected request id: ${changeReq.id} of ${changeReq.participantId}`,
                ...
                if (pendingApprovals.ipChangeRequests) {
                    for (const changeReq of pendingApprovals.ipChangeRequests) {
                        try {
                            await this.rejectParticipantSourceIpChangeRequest(secCtx, changeReq.participantId, changeReq.id);
                            messages.push({
                                reqId: changeReq.id,
                                status: "success",
                                message: `Successfully rejected request id: ${changeReq.id} of ${changeReq.participantId}`,
                ...
                if (pendingApprovals.contactInfoChangeRequests) {
                    for (const changeReq of pendingApprovals.contactInfoChangeRequests) {
                        try {
                            await this.rejectParticipantContactInfoChangeRequest(secCtx, changeReq.participantId, changeReq.id);
                            messages.push({
                                reqId: changeReq.id,
                                status: "success",
                                message: `Successfully rejected request id: ${changeReq.id} of ${changeReq.participantId}`,
    ...
    async createCSRRequest(secCtx: CallSecurityContext, participantId: string, csr: string): Promise<string> {
        ...
        const csrRequest: IParticipantCSRRequest = {
            ...
            subject: uploadResult.decodedCsrInfo?.subject!,
            requestState: ApprovalRequestState.CREATED,
            createdBy: secCtx.username!,
            createdDate: Date.now(),
            approvedBy: null,
    ...
    async approveCSRRequest(secCtx: CallSecurityContext, participantId: string, csrId: string): Promise<void> {
        ...
        const csrRequest = existing.csrRequests.find(csr => csr.csrId === csrId);
        if (!csrRequest) {
        ...
        const cert = await this._keyManagementClient.createCertificateFromCSR(csrId);
        if (!cert) {
            throw new Error("Failed to create certificate from CSR");
        }

        const certificate: IParticipantCertificate = {
            certId: cert.id!,
            fingerprint: cert.keyFingerprint,
            subject: cert.decodedCertInfo?.subject!,
            usedFor: "JWS",
            validTo: cert.decodedCertInfo?.validTo!,
            validFrom: cert.decodedCertInfo?.validFrom!,
            createdBy: secCtx.username!,
            createdDate: cert.createdDate!,
        }

        if (!existing.certificates) {
        ...
        this._auditClient.audit(
            AuditedActionNames.PARTICIPANT_CSR_REQUEST_APPROVED,
            true,
            this._getAuditSecCtx(secCtx),
            [
                { key: "participantId", value: participantId },
    ...
    async rejectCSRRequest(secCtx: CallSecurityContext, participantId: string, csrId: string): Promise<void> {
        ...
        if (secCtx && csrRequest.createdBy === secCtx.username) {
            ...
            throw new MakerCheckerViolationError(
                "Maker check violation - Same user cannot create and reject participant CSR request"
            );
        }

        csrRequest.requestState = ApprovalRequestState.REJECTED;
        csrRequest.rejectedBy = secCtx.username;
        ...
        this._auditClient.audit(
            AuditedActionNames.PARTICIPANT_CSR_REQUEST_REJECTED,
            true,
            this._getAuditSecCtx(secCtx),
            [
                { key: "participantId", value: participantId },
